// * File ini menyimpan implementasi legacy service module submissions sebagai referensi transisi.

import prisma from "../../config/prisma";
import { DEFAULT_TIMEZONE } from "../../config/timezone";
import {
  findFirstInvalidScheduleDateInRange,
  type ScheduleRangeValidationIssue,
...
const REJECTED_DELETE_WINDOW_MS = 48 * 60 * 60 * 1000;

function toStartOfDay(input: string | Date) {
...
function toEndOfDay(input: string | Date) {
  const date = new Date(input);
  if (Number.isNaN(date.getTime())) {
    throw new Error("Bad Request: Format tanggal tidak valid.");
  }
...
function toScheduleConflictMessage(issue: ScheduleRangeValidationIssue) {
